...
public class GL10 {
    ...
    public void glViewport(int x, int y, int width, int height) { GLES20.glViewport(x,y,width,height); }
    public void glEnable(int cap) { if (cap != GL_TEXTURE_2D) GLES20.glEnable(cap); }
    public void glDisable(int cap) { if (cap != GL_TEXTURE_2D) GLES20.glDisable(cap); }
    public void glBlendFunc(int sfactor, int dfactor) { GLES20.glBlendFunc(sfactor, dfactor); }
    public void glDepthFunc(int func) { GLES20.glDepthFunc(func); }
    public void glHint(int target, int mode) { }
    public void glShadeModel(int mode) { }
    public void glPolygonOffset(float factor, float units) { GLES20.glPolygonOffset(factor, units); }
    public void glLineWidth(float width) { lineWidth = width; GLES20.glLineWidth(width); }
    public void glPointSize(float size) { pointSize = size; }

    public void glMatrixMode(int mode) { matrixMode = mode; }
    public void glLoadIdentity() { Matrix.setIdentityM(currentMatrix(), 0); }
    public void glPushMatrix() { currentStack().push(currentMatrix().clone()); }
    public void glPopMatrix() { if (!currentStack().isEmpty()) System.arraycopy(currentStack().pop(), 0, currentMatrix(), 0, 16); }
    public void glTranslatef(float x, float y, float z) { Matrix.translateM(currentMatrix(), 0, x, y, z); }
    public void glRotatef(float angle, float x, float y, float z) { Matrix.rotateM(currentMatrix(), 0, angle, x, y, z); }
    public void glScalef(float x, float y, float z) { Matrix.scaleM(currentMatrix(), 0, x, y, z); }
    ...
    public void glLoadMatrixf(float[] m, int offset) { System.arraycopy(m, offset, currentMatrix(), 0, 16); }
    public void glGetFloatv(int pname, float[] params, int offset) {
        if (pname == GL_MODELVIEW_MATRIX) {
            System.arraycopy(modelViewMatrix, 0, params, offset, 16);
        }
    }
    public void setProjectionMatrix(float[] m) { System.arraycopy(m, 0, projectionMatrix, 0, 16); }
    public void setModelViewMatrix(float[] m) { System.arraycopy(m, 0, modelViewMatrix, 0, 16); }
    ...
    public void glVertexPointer(int size, int type, int stride, Buffer pointer) {
        vertexSize = size; vertexStride = stride; vertexPointer = pointer;
    }
    public void glColorPointer(int size, int type, int stride, Buffer pointer) {
        colorSize = size; colorStride = stride; colorPointer = pointer;
    }
    public void glTexCoordPointer(int size, int type, int stride, Buffer pointer) {
        texCoordSize = size; texCoordStride = stride; texCoordPointer = pointer;
    }
    public void glColor4f(float r, float g, float b, float a) {
        currentColor[0]=r; currentColor[1]=g; currentColor[2]=b; currentColor[3]=a;
    }

    public void glBindTexture(int target, int texture) { GLES20.glBindTexture(target, texture); }
    public void glGenTextures(int n, int[] textures, int offset) { GLES20.glGenTextures(n, textures, offset); }
    public void glDeleteTextures(int n, int[] textures, int offset) { GLES20.glDeleteTextures(n, textures, offset); }
    public void glTexParameterf(int target, int pname, float param) { GLES20.glTexParameterf(target, pname, param); }
    public void glTexImage2D(int target, int level, int internalformat, int width, int height, int border, int format, int type, Buffer pixels) {
        GLES20.glTexImage2D(target, level, internalformat, width, height, border, format, type, pixels);
    }

    public void glGenBuffers(int n, int[] ids, int offset) { GLES20.glGenBuffers(n, ids, offset); }
    public void glBindBuffer(int target, int buffer) { boundArrayBuffer = buffer; GLES20.glBindBuffer(target, buffer); }
    public void glBufferData(int target, int size, Buffer data, int usage) { GLES20.glBufferData(target, size, data, usage); }
    public void glDeleteBuffers(int n, int[] ids, int offset) { GLES20.glDeleteBuffers(n, ids, offset); }
