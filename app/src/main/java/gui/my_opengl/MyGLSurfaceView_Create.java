...
public class MyGLSurfaceView_Create  extends GLSurfaceView {
    ...
    private MyGLRenderer_Create renderer;
    ...
    public MyGLSurfaceView_Create(Context context) {
        super(context);
        init(context);
    }

    public MyGLSurfaceView_Create(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        setEGLConfigChooser(new MyGLSurfaceView_Create.MultisampleConfigChooser());


        renderer = new MyGLRenderer_Create();
        setRenderer(renderer);
        setRenderMode(GLSurfaceView.RENDERMODE_CONTINUOUSLY);

        gestureDetector = new GestureDetector(context, new MyGLSurfaceView_Create.GestureListener());
        scaleDetector = new ScaleGestureDetector(context, new MyGLSurfaceView_Create.ScaleListener());
    }
    ...
    private void handleRotation(float dx, float dy) {
        if (DataSaved.lock3dRotation > 0) {
            MyGLRenderer.angleY_extra += dx * ROTATION_SENSITIVITY;
        } else {
            MyGLRenderer.angleY += dx * ROTATION_SENSITIVITY;
        }

        angleX += dy * ROTATION_SENSITIVITY;
        angleX = Math.max(-110f, Math.min(0f, angleX));
    }

    private void handlePan(float dx, float dy) {
        if (renderer.is3D) {
            MyGLRenderer.panX += dx * PAN_FACTOR_3D;
            MyGLRenderer.panY -= dy * PAN_FACTOR_3D;
        } else if (renderer.is2D) {
            float safeScale = Math.max(0.0001f, scale_2d);
            float panFactor = (PAN_FACTOR_2D_BASE * orthoBaseSize) / safeScale;
            MyGLRenderer.panX_2d += dx * panFactor;
            MyGLRenderer.panY_2d -= dy * panFactor;
        }
    }
