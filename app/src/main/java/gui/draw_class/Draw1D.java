...
public class Draw1D extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            double mDeltaX = 0, mDeltaZ = 0;

            if (Digging1D.flagLaser_D1D) {
            ...
            if (DataSaved.projectionFlag == 0) {
                ...
                paint.setColor(MyColorClass.colorConstraint);
                paint.setStrokeWidth(3);
                mX = (float) (dist * Math.cos(Math.toRadians(90 + DataSaved.slopeY)));
                mY = (float) (dist * Math.sin(Math.toRadians(90 + DataSaved.slopeY)));
