...
public class DrawDXF_Layer1 extends View {
    ...
    public DrawDXF_Layer1(Context context) {
        super(context);
        paint = new Paint();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            stick.add(new PointF(bucket.get(0).x, bucket.get(0).y));
...
//-------------------------------- DRAW BUCKET --------------------------------
            ...
            if (DataSaved.isWL == EXCAVATOR) {
                ...
                canvas.drawCircle(bucket.get(0).x, bucket.get(0).y, radius * 0.40f, paint);
            } else {
                canvas.drawCircle(stick.get(0).x, stick.get(0).y, (float) (radiusStick * 0.2), paint);
                ...
                float xLinea = bucket.get(0).x - (float) (offsetAttacco * Math.cos(Math.toRadians(90 + 360 - bucketAngle)));
                float yLinea = bucket.get(0).y - (float) (offsetAttacco * Math.sin(Math.toRadians(90 + 360 - bucketAngle)));
                ...
                canvas.drawLine(bucket.get(2).x, bucket.get(2).y, bucket.get(1).x, bucket.get(1).y, paint);
            }
    ...
    private float worldToFrontX(
        ...
        double yawRad = Math.toRadians(hdt_BOOM);
