...
public class Draw2D_Layer2_Tilt extends View {

    private final Paint paint;
    public float offsetY;


    public Draw2D_Layer2_Tilt(Context context) {
        super(context);
        paint = new Paint();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        try {
            ...
            float scala, scala2, scala3;
            scala = (float) DataSaved.scale_Factor;
            scala2 = (float) ((float) (DataSaved.L_Bucket * 0.5 * scala) * 0.15);
            scala3 = (float) ((float) (DataSaved.W_Bucket * 0.5 * scala) * 0.1);
            Path path = new Path();
            ...
            double bucketHeight = DataSaved.L_Bucket-DataSaved.L_Tilt;
            ...
            if (ExcavatorLib.correctTilt < -90 || ExcavatorLib.correctTilt > 90) {
            ...
            left_top_tilt.x = ((pointsStick.get(6).x + pointsStick.get(4).x) / 2f) - stickWidth * 1.35f;
            left_top_tilt.y = ((pointsStick.get(6).y + pointsStick.get(4).y) / 2f) - (float) ly;

            right_bottom_tilt.x = ((pointsStick.get(6).x + pointsStick.get(4).x) / 2f) + stickWidth * 1.35f;
            right_bottom_tilt.y = ((pointsStick.get(6).y + pointsStick.get(4).y) / 2f) ;
            ...
            if ((ExcavatorLib.correctTilt >= (ExcavatorLib.actualX2D - DataSaved.deadbandFlatAngle)) && (ExcavatorLib.correctTilt <= (ExcavatorLib.actualX2D + DataSaved.deadbandFlatAngle))) {

                paint.setColor(Color.GREEN);
                paint.setStrokeWidth(5f);
                float mGroundA = (pointA_bottom.y - pointB_bottom.y) / (pointA_bottom.x - pointB_bottom.x);
                float qGroundA = -pointA_bottom.x * mGroundA + pointA_bottom.y;

                float mGroundC = (pointB_bottom.y - pointC_bottom.y) / (pointB_bottom.x - pointC_bottom.x);
                float qGroundC = -pointB_bottom.x * mGroundC + pointB_bottom.y;


                float distanceFlatTilt = (float) (bucketWidth * 10.20f);


                float aYFlat = (float) (mGroundA * (pointA_bottom.x - distanceFlatTilt * scala) + qGroundA);
                float cYFlat = (float) (mGroundC * (pointC_bottom.x + distanceFlatTilt * scala) + qGroundC);

                canvas.drawLine((float) (pointA_bottom.x - distanceFlatTilt * scala), aYFlat, pointB_bottom.x, pointB_bottom.y, paint);
                canvas.drawLine(pointB_bottom.x, pointB_bottom.y, (float) (pointC_bottom.x + distanceFlatTilt * scala), cYFlat, paint);
            }


            float m = (float) bucketHeight * 0.15f;
            ///
            //denti
            paint.setColor(MyColorClass.colorBucket);
            ...
            float mGroundC = (pointB_bottom.y - pointC_bottom.y) / (pointB_bottom.x - pointC_bottom.x);
            float qGroundC = -pointB_bottom.x * mGroundC + pointB_bottom.y;
            float distanceFlatTilt = (float) (bucketWidth * 0.01f);
            float aYFlat = (float) (mGroundA * (pointA_bottom.x - distanceFlatTilt * scala) + qGroundA);
            float cYFlat = (float) (mGroundC * (pointC_bottom.x + distanceFlatTilt * scala) + qGroundC);

            canvas.drawLine((float) (pointA_bottom.x - distanceFlatTilt * scala), aYFlat - (m / 2), pointB_bottom.x, pointB_bottom.y - (m / 2), paint);
            canvas.drawLine(pointB_bottom.x, pointB_bottom.y - (m / 2), (float) (pointC_bottom.x + distanceFlatTilt * scala), cYFlat - (m / 2), paint);
            ...
            canvas.drawText(Utils.readAngoloLITE(String.valueOf(ExcavatorLib.correctTilt)) + Utils.getGradiSimbol(), pointB_bottom.x, pointB_bottom.y - (20), paint);
