...
static jdoubleArray
transformWithPJ(JNIEnv *env, ProjHolder *holder, PJ *transform, double x, double y, double z) {
    if (!transform) {
...
Java_packexcalib_gnss_NativeProjTransformer_nativeFromPipeline(
    ...
    PJ *transform = normalizeIfPossible(holder->ctx, raw);
    if (!transform) {
        setLastErrorFromContext(holder,
                                "proj_normalize_for_visualization(pipeline) one-shot fallita");
        return nullptr;
    }
